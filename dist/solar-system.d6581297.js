...
})({"gDoe8":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "da3011c9d6581297";
"use strict";
...
},{}],"97tmj":[function(require,module,exports,__globalThis) {
/**
 * @module solar-system
 ...
 * Модель Солнца
 * @constant {THREE.Mesh}
 */ const sunGeometry = new _three.SphereGeometry(5, 32, 32);
 ...
 */ const sunLight = new _three.PointLight(0xffd700, 2, 300);
scene.add(sunLight);
/**
 * Свечение Солнца (полупрозрачная сфера)
 * @constant {THREE.Mesh}
 */ const sunGlowGeometry = new _three.SphereGeometry(5.5, 32, 32);
const sunGlowMaterial = new _three.MeshBasicMaterial({
    color: 0xff4500,
    transparent: true,
    opacity: 0.3
});
const sunGlow = new _three.Mesh(sunGlowGeometry, sunGlowMaterial);
scene.add(sunGlow);
/**
 ...
 */ planets.forEach((planet)=>{
    const geometry = new _three.SphereGeometry(planet.radius, 32, 32);
    const material = new _three.MeshLambertMaterial({
        color: planet.color
    });
    const mesh = new _three.Mesh(geometry, material);
    const orbitGeometry = new _three.RingGeometry(planet.distance - 0.1, planet.distance + 0.1, 64);
    const orbitMaterial = new _three.MeshBasicMaterial({
        color: 0xFFFFFF,
        side: _three.DoubleSide,
        transparent: true,
        opacity: 0.1
    });
    const orbit = new _three.Mesh(orbitGeometry, orbitMaterial);
 ...
 * Кольца Сатурна
 * @constant {THREE.Mesh}
 */ const saturnRingGeometry = new _three.RingGeometry(2.2, 3.5, 32);
...
for(let i = 0; i < asteroidCount; i++){
    ...
    const spriteMaterial = new _three.SpriteMaterial({
        map: asteroidTextures[Math.floor(Math.random() * asteroidTextures.length)],
        transparent: true,
        opacity: 0.8 + Math.random() * 0.2
    });
    const sprite = new _three.Sprite(spriteMaterial);
...
const outerOortGeometry = new _three.BufferGeometry();
...
outerOortGeometry.setAttribute('position', new _three.BufferAttribute(outerPositions, 3));
outerOortGeometry.setAttribute('color', new _three.BufferAttribute(outerColors, 3));
const outerOortMaterial = new _three.PointsMaterial({
...
const outerOortCloud = new _three.Points(outerOortGeometry, outerOortMaterial);
...
},{"three":"dsoTF","three/examples/jsm/controls/OrbitControls.js":"45ipX"}]},["gDoe8","97tmj"], "97tmj", "parcelRequireae6e", {})

//# sourceMappingURL=solar-system.d6581297.js.map
